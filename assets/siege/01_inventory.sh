...
URL_GET="${URL_HOST}/inventory/product/GPU-${RANDOM_INV}"
...
if echo "$STATUS_CODE" | grep -q "HTTP:200"; then
  	echo "HTTP:200 GET /inventory/product/BEER-${RANDOM_INV}"
elif echo "$STATUS_CODE" | grep -q "HTTP:404"; then	
	echo -e "\e[38;2;255;165;0m** ERROR $STATUS_CODE ==> /inventory/product/BEER-${RANDOM_INV}\e[0m"
else
	echo -e "\e[31m** ERROR $STATUS_CODE ==> /inventory/product/BEER-${RANDOM_INV}\e[0m"
fi

#------------------------------------------
RANDOM_INV=$((RANDOM % 999 + 1))

URL_PUT="${URL_HOST}/inventory/product/BEER-${RANDOM_INV}"
...
if echo "$STATUS_CODE" | grep -q "HTTP:200"; then
  	echo "HTTP:200 PUT inventory/product/BEER-${RANDOM_INV}"
elif echo "$STATUS_CODE" | grep -q "HTTP:404"; then	
	echo -e "\e[38;2;255;165;0m** ERROR $STATUS_CODE ==> inventory/product/BEER-${RANDOM_INV}\e[0m"	
else
	echo -e "\e[31m** ERROR $STATUS_CODE ==> inventory/product/BEER-${RANDOM_INV}\e[0m"
